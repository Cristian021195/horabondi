...
export default function handler(
  ...
  res: NextApiResponse
    ...
    try {
        ...
        let files_arr = fs.readdirSync(process.cwd()+DIRECTORIES.JSON_DIR_PRECIOS).filter((dir:string,dir_i:number)=>dir.includes(req.query.empresa+''));
        let data = files_arr.map((fa,fa_i)=>{
            return {...JSON.parse(fs.readFileSync(process.cwd()+DIRECTORIES.JSON_DIR_PRECIOS+'/'+fa, {encoding:'utf-8'})), data_file:fa.replace(REGEX.DATA_TEXT,"")}
        })
